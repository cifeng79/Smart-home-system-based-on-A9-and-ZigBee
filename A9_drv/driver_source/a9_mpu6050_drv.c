...
int mpu6050_gpio_init(void)
{
	...
	mpu6050_write(MPU_SAMPLE_RATE_REG, 0x07);//设置采样率50Hz       
...
long mpu6050_ioctl(struct file *filp, unsigned int cmd, unsigned long args)
{
	...
	switch(cmd)
	{
		case MPU6050_GYRO:
			...
			data.gyro.y =  mpu6050_read(MPU_GYRO_YOUTL_REG);	
			data.gyro.y |= mpu6050_read(MPU_GYRO_YOUTH_REG) << 8;	
			
			data.gyro.z =  mpu6050_read(MPU_GYRO_ZOUTL_REG);	
			data.gyro.z |= mpu6050_read(MPU_GYRO_ZOUTH_REG) << 8;	
			break;
...
static int mpu6050_probe(struct i2c_client *client,const struct i2c_device_id *devid)
{
	printk("---->%s--->%d\n",__func__,__LINE__);
	mpu_client = client;
...
static int mpu6050_remove(struct i2c_client *mpu_6050)
{
	printk("---->%s--->%d\n",__func__,__LINE__);
	return 0;
}
